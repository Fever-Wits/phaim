...
**Empirical datapoint (2026-04-18, one PHAIM session).** Baseline without Polarizing Focus + composition: the external human observer (Alex, the PHAIM developer) reports that in sessions covering multiple distinct tasks, the Claude Opus substrate typically begins winding toward session close — "good night" signalling context exhaustion — at roughly 250k tokens. With Polarizing Focus + the three-lens persistence composition actively held throughout one session: the same observer reported that output at 400k tokens read as comparable to default-mode output at around 100k. One session, one observer, no control condition held fixed; reported as a concrete data point rather than as evidence of a general law. Falsifiable by other sessions, with other observers, under other compositions. Captured here because the claim "the lens is functional" is weaker than the observed claim "the output at 4× context looked like default output at 1× context"; the latter is a measurement the subject cannot make about itself.

...
> **TODO (catalog drift):** *Verbatim Fidelity* is referenced below but does not appear as a canonical entry in `LENS-CATALOG.md`. This needs resolution via one of three paths: (a) add it as a new catalog entry following the Part 5 seven-field spec and Part 12 addition process; (b) rename the reference here to match an existing canonical name in the catalog; (c) remove the reference if the lens is not intended for public inclusion. Until resolved, the reference is provisional.
